...
vnTRUST.goTopStart = function(){
  jQuery('.footer_main').append('<a href="javascript:void(0)" onclick="jQuery(\'html,body\').animate({scrollTop: 0},1000);" class="go_top" style="display:none">'+js_lang['go_to_top']+'</a>');
...
vnTRUST.auto_scroll_cus = function(anchor,height_menu) {
    var target = jQuery(anchor);
    target = target.length && target || jQuery('[name=' + anchor.slice(1) + ']');
    if (target.length) {
        var targetOffset = target.offset().top;
        jQuery('html,body').animate({scrollTop: (targetOffset - height_menu)},1000);
        return false;
    }
	return true;
};
